...
import { describe, expect, test, TestCase } from "@desk-framework/frame-test";
...
describe("LazyString", () => {
	...
	test("strf creates LazyString", (t) => {
		t.log("With string argument");
		expect(strf("Hello")).toBeInstanceOf(LazyString);
		t.log("With number argument");
		expect(strf(123)).toBeInstanceOf(LazyString);
		t.log("With undefined argument");
		expect(strf(undefined as any)).toBeInstanceOf(LazyString);
	});

	test("strf can be used with template literal", (t) => {
		t.log("With single string");
		expect(strf`foo`).toBeInstanceOf(LazyString);
		expect(strf`foo`)
			.asString()
			.toBe("foo");
		t.log("With parameters");
	...
	describe("Formatting", () => {
		...
		test("Retrieve format arguments", () => {
			...
			expect(ls).asString().toBe("foo 123");
			let args = ls.getFormatArgs();
			expect(args).toBeArray(2);
			expect(args[0]).toHaveProperty("foo").toBe("foo");
		...
		describe("Number/pad/length formatting", () => {
				...
				for (let type of types) {
					...
					for (let width of ["", "1", "4", "8"]) {
						...
						for (let flag of flags) {
							t.log("Checking %" + flag + width + prec + type, n);
			...
			test("%d/i/u, n/N with whole number", (t) => {
				match(
					t,
