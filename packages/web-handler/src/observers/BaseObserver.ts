...
export abstract class BaseObserver<TUIViewElement extends UIElement> {
	...
	private _remounted = WebRenderer.lastRemountIdx;
	...
	abstract getOutput(): RenderContext.Output<HTMLElement>;
	...
	/** Called before handling DOM events, for some elements (see `events.ts`); the view has already been checked here and must be defined; return `false` to stop the event from being handled by the view */
	onDOMEvent(event: Event, data: any): boolean | void {
		// nothing here
	}
