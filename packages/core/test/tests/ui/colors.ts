...
describe("UIColor colors", (ctx) => {
	...
	describe("Base colors", () => {
		test("Color value using constructor", () => {
			expect(new UIColor("Foo")).asString().toBe("Foo");
			expect(new UIColor("#000000")).asString().toBe("#000000");
		});

		test("Color value using predefined instance", () => {
			expect(ui.color.BLACK).asString().toBe("#000000");
		});

		test("Color value using indirectly predefined instance", () => {
			let blue = ui.color("Blue").toString();
			expect(ui.color.PRIMARY).asString().toBe(blue);
			expect(ui.color.TEXT).asString().toBe("#000000");
		});
	});
	...
	describe("Brightness and text color", () => {
		...
		test("Foreground selection", () => {
			...
			expect(lightFg).asString().toBe(ui.color.RED.toString());
			let darkFg = ui.color.BLACK.fg(ui.color.RED, ui.color.GREEN);
			expect(darkFg).asString().toBe(ui.color.GREEN.toString());
		});
	});
	...
	describe("Color mixing", () => {
		test("Single mix, hex with transparent", () => {
			...
			expect(red.mix(none, 0)).asString().toBe("rgb(255,0,0)");
			expect(red.mix(none, 1)).asString().toBe("rgba(0,0,0,0)");
			expect(red.mix(none, 0.5)).asString().toBe("rgba(128,0,0,0.5)");
			expect(none.mix(red, 0.5)).asString().toBe("rgba(128,0,0,0.5)");
		});

		test("Single mix, hex", () => {
			let red = new UIColor("#f00");
			let blue = new UIColor("#00f");
			expect(red.mix(blue, 0)).asString().toBe("rgb(255,0,0)");
			expect(red.mix(blue, 1)).asString().toBe("rgb(0,0,255)");
			expect(red.mix(blue, 0.5)).asString().toBe("rgb(128,0,128)");
		});
		...
		test("Single mix, rgba", () => {
			...
			expect(red.mix(blue, 0)).asString().toBe("rgba(255,0,0,0.1)");
			expect(red.mix(blue, 1)).asString().toBe("rgba(0,0,255,0.2)");
			expect(red.mix(blue, 0.5)).asString().toBe("rgba(128,0,128,0.15)");
		});

		test("Alpha, hex", () => {
			let black = new UIColor("#000000");
			let color = new UIColor("#123");
			expect(black.alpha(1)).asString().toBe("rgb(0,0,0)");
			expect(color.alpha(1)).asString().toBe("rgb(17,34,51)");
			expect(black.alpha(0)).asString().toBe("rgba(0,0,0,0)");
			expect(color.alpha(0)).asString().toBe("rgba(17,34,51,0)");
			expect(black.alpha(0.5)).asString().toBe("rgba(0,0,0,0.5)");
			expect(color.alpha(0.5)).asString().toBe("rgba(17,34,51,0.5)");
		});
		...
		test("Alpha, rgba", () => {
			...
			expect(black.alpha(1)).asString().toBe("rgba(0,0,0,0.1)");
			expect(color.alpha(1)).asString().toBe("rgba(17,34,51,0.2)");
			expect(black.alpha(0)).asString().toBe("rgba(0,0,0,0)");
			expect(color.alpha(0)).asString().toBe("rgba(17,34,51,0)");
			expect(black.alpha(0.5)).asString().toBe("rgba(0,0,0,0.05)");
			expect(color.alpha(0.5)).asString().toBe("rgba(17,34,51,0.1)");
		});

		test("Lum, hex", () => {
			let grey1 = new UIColor("#222"); // 0x22 = 34
			let grey2 = new UIColor("#ccc"); // 0xCC = 204
			expect(grey1.brighten(1)).asString().toBe("rgb(255,255,255)");
			expect(grey1.brighten(-1)).asString().toBe("rgb(0,0,0)");
			expect(grey1.brighten(0)).asString().toBe("rgb(34,34,34)");
			expect(grey1.brighten(0.5)).asString().toBe("rgb(145,145,145)");
			expect(grey1.brighten(-0.5)).asString().toBe("rgb(17,17,17)");
			expect(grey2.brighten(0.5)).asString().toBe("rgb(230,230,230)");
			expect(grey2.brighten(-0.5)).asString().toBe("rgb(102,102,102)");
		});
		...
		test("Lum, rgba", () => {
			...
			expect(grey1.brighten(1)).asString().toBe("rgba(255,255,255,0.5)");
			expect(grey1.brighten(-1)).asString().toBe("rgba(0,0,0,0.5)");
			expect(grey1.brighten(0)).asString().toBe("rgba(34,34,34,0.5)");
			expect(grey1.brighten(0.5)).asString().toBe("rgba(145,145,145,0.5)");
			expect(grey1.brighten(-0.5)).asString().toBe("rgba(17,17,17,0.5)");
			expect(grey2.brighten(0.5)).asString().toBe("rgba(230,230,230,0.5)");
			expect(grey2.brighten(-0.5)).asString().toBe("rgba(102,102,102,0.5)");
		});
