...
export class FileParser {
	constructor(
		...
		this._tokenizer = new Tokenizer(fileName, fileContent);
	...
	parse() {
		...
		if (result.next < this._tokens.length) {
			let token = this._tokens[result.next];
			throw Error(
	...
	expectNamespace(cur: number): ParseResult {
		...
		let name = this._tokens[cur + 1]!.content;
		let start = cur;
		cur += 2;
	...
	expectClassDefinition(cur: number): ParseResult {
		...
		while (
			...
			extendsNames.push(
				Tokenizer.toSource(this._tokens.slice(cur, parsedType.next)),
			);
			cur = parsedType.next;
		}
	...
	expectPropertyMember(cur: number): ParseResult {
		...
		let name = Tokenizer.toSource(this._tokens.slice(start, cur));
	...
	expectMethod(cur: number): ParseResult {
		...
		let name =
			cur > start
				? Tokenizer.toSource(this._tokens.slice(start, cur))
				: "[call]";
	...
	private _throwParseError(cur: number, expected?: string): undefined {
		let token = this._tokens[cur];
		if (token?.type === TokenType.JSDoc) token = this._tokens[cur + 1];
	...
	private _log(cur: number, name: string) {
		if (!this._enableDebugOutput) return;
		let token = this._tokens[cur];
		let content = token?.content || "<no content>";
		if (content.length > 80) content = content.slice(0, 77) + "...";
	...
	private _enableDebugOutput?: boolean;
	private _tokenizer: Tokenizer;
	private _tokens: ReadonlyArray<TokenInfo> = [];
}
